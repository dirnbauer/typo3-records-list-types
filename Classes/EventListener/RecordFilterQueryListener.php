...
use Webconsulting\RecordsListTypes\Service\LlmRecordSearchService;
...
final readonly class RecordFilterQueryListener
{
    ...
    public function __invoke(ModifyDatabaseQueryForRecordListingEvent $event): void
    {
        ...
        $filters = $this->configurationService->getFiltersForTable($table, $event->getPageId());
        if ($filters === []) {
            return;
        }
        ...
        foreach ($filters as $filter) {
            ...
            $this->applyFilter($queryBuilder, $table, $event->getPageId(), $filter, $values[$id]);
        }
    }
