...
- Restore the 1.2.0 Records module interface after the 1.3.0 UI overhaul.
  Grid cards again show their field labels and values in the compact card body;
  List, Grid, Compact and Teaser remain selectable in the View dropdown.
- Restore the earlier filter panel spacing. The 1.3.0 panel used
  `--typo3-recordlist-padding-*` outside `.recordlist`, where TYPO3 does not
  define those variables, so its padding disappeared.
- Release in lockstep with Records List Examples 2.0.0. The companion package
  requires this exact version so future releases cannot silently drift.
