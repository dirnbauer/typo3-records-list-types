...
..  confval:: mod.web_list.filters.table.<table>.<filter>.fields
    ...
    Fields searched by ``text`` and ``llm`` filters. Non-existing fields are
    skipped.
...
..  list-table::
    ...
    *   -   ``llm`` or ``ai``
        -   ``llm``
        -   Uses optional EXT:nr_llm-backed record search.
...
.. _configuration-filters-llm:

LLM search filter
=================

The ``llm`` filter is not a classic field filter. It provides a textarea where
editors can ask a question about the current table's candidate records. The
...
..  confval:: mod.web_list.filters.table.<table>.llm.configurationIdentifier
    :name: conf-filters-llm-configuration-identifier
    :type: string
    :default: ``record-list-search``

    Identifier of an EXT:nr_llm LLM configuration record. The older
    :typoscript:`configuration` key is accepted as an alias.

..  confval:: mod.web_list.filters.table.<table>.llm.candidateLimit
    :name: conf-filters-llm-candidate-limit
    :type: int
    :default: ``80``

    Maximum number of candidate records sent to EXT:nr_llm.

..  confval:: mod.web_list.filters.table.<table>.llm.resultLimit
    :name: conf-filters-llm-result-limit
    :type: int
    :default: ``25``

    Maximum number of matching UIDs requested from EXT:nr_llm.

The LLM filter is hidden and a warning is shown when:

-   EXT:nr_llm is not installed.
-   No ``configurationIdentifier`` is configured.
-   The referenced nr_llm configuration cannot be found or is inactive.
-   nr_llm has no available provider.
